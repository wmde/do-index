...
function loadSurveyForm(slug, elementId, cantEnter, canView, open, needLogin) {
  ...
  $.getJSON(url, function(survey, status) {
    ...
    if (cantEnter) {
      ...
      if (needLogin == true) {
        div.text("Sie müssen angemeldet sein, um diese Umfrage ausfüllen zu können.");
      //} else if (open == true) {
      //  div.text("Diese Umfrage haben Sie bereits ausgefüllt.");
      //} else {
      //  div.text("Diese Umfrage steht noch nicht bereit.");
      }
    } else {
      if (isPoll) {
  ...
  // Fix the back button for the surveys
